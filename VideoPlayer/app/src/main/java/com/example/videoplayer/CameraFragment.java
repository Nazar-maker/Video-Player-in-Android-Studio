...
public class CameraFragment extends Fragment
    ...
    private static final String TAG = "VideoPlayerCamera";
    ...
    private TextView result, volume_text;
    ...
    public CameraFragment(SimpleExoPlayer player) {
     ...
     *
     */
    private void showToast(String s, TextView tv) {
        SpannableStringBuilder builder = new SpannableStringBuilder();
        SpannableString str1 = new SpannableString(s);
        builder.append(str1);
        showToast(builder, tv);
    }

    private void showToast(SpannableStringBuilder builder, TextView tv) {
    ...
    public static CameraFragment newInstance(SimpleExoPlayer player) {
        return new CameraFragment(player);
    }
    ...
    public void onViewCreated(final View view, Bundle savedInstanceState) {
        textureView = view.findViewById(R.id.cameraDisplay);
        result = view.findViewById(R.id.result);
        audioManager = (AudioManager) getActivity().getSystemService(Context.AUDIO_SERVICE);
        linearLayout = getActivity().findViewById(R.id.vol_container);
        volume_text = getActivity().findViewById(R.id.volume_text);
    ...
    private void openCamera(int width, int height) {
        ...
        try {
            ...
            if (ActivityCompat.checkSelfPermission(getContext(), Manifest.permission.CAMERA) == PackageManager.PERMISSION_DENIED) {
                ...
                Toast.makeText(getContext(), "Camera is open", Toast.LENGTH_SHORT).show();
                return;
            }
...
//            SurfaceTexture texture = dummyTextureView.getSurfaceTexture();
            assert texture != null;
    ...
    private void classifyFrame() {
        if (classifier == null || getActivity() == null || cameraDevice == null) {
            showToast("Uninitialized Classifier or invalid context.", result);
            return;
        }
        ...
        Matrix matrix = new Matrix();
        if (rotation == 1) matrix.postRotate(-90);
        else if (rotation == 3) matrix.postRotate(90);
...
//                    });
//        }
        String token = textToShow.toString();
        Activity activity = getActivity();
        activity.runOnUiThread(
                new Runnable() {
                    @Override
        ...
        showToast(textToShow, result);
    }

    //TODO
    private void controls(String token) {
        if (!token.equalsIgnoreCase("9") || !token.equalsIgnoreCase("2")) {
        ...
        if (token.equalsIgnoreCase("0")) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                if(value) {
                    ...
                    volume_text.setText(" " + (int)volPer + "%");
